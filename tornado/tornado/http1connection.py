...
class HTTP1Connection(httputil.HTTPConnection):
    ...
    def read_response(self, delegate):
        ...
        print('read_response', type(delegate))
    ...
    def _read_message(self, delegate):
        ...
        try:
            ...
            print('start_line:', start_line, 'headers', type(headers))
            ...
            if not skip_body:
                ...
                print('params', self.params.body_timeout)
...
class HTTP1ServerConnection(object):
    ...
    def start_serving(self, delegate):
        ...
        print('-serving_future', self._serving_future.done())
        # Register the future on the IOLoop so its errors get logged.
        self.stream.io_loop.add_future(self._serving_future,
                                       lambda f: f.result())

    @gen.coroutine
    def _server_request_loop(self, delegate, func_name='server_request_loop'):
        try:
            while True:
                conn = HTTP1Connection(self.stream, False,
                                       self.params, self.context)
                request_delegate = delegate.start_request(self, conn)
                print('request_delegate', type(request_delegate))
                try:
